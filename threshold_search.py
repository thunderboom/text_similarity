...
class NewsConfig:
    ...
    def __init__(self):
        ...
        self.save_path = [absdir + '/model_saved' + '/' + self.task + '/' + model_name_try + '/' for model_name_try in
                          self.models_name]
        self.dev_split = 0.1
        self.test_split = 0.1
        self.seed = 369
        ...
        # test data path
        self.test_data_dir = './real_data/train_20200228.csv'
        # model vote
...
def thucNews_task(config):
    ...
    best_threshold = 0
    best_acc = 0
    true_label = np.asarray(pd.read_csv('./real_data/train_20200228.csv')['label'], dtype=np.int)
    for threshold in np.linspace(0.3, 0.6, num=151):
        predict_label = np.asarray(predict_label > threshold, dtype=np.int)
        true_acc = np.mean(true_label == predict_label)
        print(threshold, true_acc)
    ...
    print("best threshold:{},best_acc:{}".format(best_threshold, best_acc))
